...
export default function Task(props) {
  ...
  let classNames = '';
  const isDanger = remainingTime < 60;
  return props.edit ? (
    ...
    <li className={props.done ? (classNames += 'completed') : classNames}>
      <div className="view">
        ...
        <label htmlFor={props.id}>
          <span className="description">{props.body}</span>
          <span className="description-info">
            ...
            <span className="created">
              <span className={clsx(isDanger ? 'text-orange-600' : 'timer')}>{formatTime(remainingTime)}</span>
            </span>
          </span>
